...
function ReviewMode({ article, refresh, close }) {
    ...
    const submitReview = async () => {
        ...
        try {
            ...
            await axios.post(`${API_BASE}/review/add`, payload, { // Adjust API_BASE and endpoint as needed
                headers: {
    ...
    return (
        <div className="my-modal">
            <div className="container bg-white overflow-y-scroll p-3" style={{ maxHeight: '90vh' }}>
                <div className="modal-content">
                    ...
                    <div className="modal-body p-4">
                        ...
                        {article.content.map((item, idx) => (
                            <div key={item._id || idx} className={`position-relative mb-3 ${item.classes}`}>
                                ...
                                <div className="d-flex justify-content-end mt-2">
                                    ...
                                    >
                                        <i className={`bi ${activeCommentIndex === idx ? 'bi-chat-left-fill' : 'bi-chat-left'}`}></i>
                                        {activeCommentIndex === idx ? ' Hide Comment' : ' Add Comment'}
                                    </button>
                                </div>
                                ...
                                {activeCommentIndex === idx && (
                                    <div
                                        ...
                                        style={{ position: 'relative', zIndex: 1050, backgroundColor: '#f8f9fa' }} // Adjust styling as needed
                                    >
                                        <label htmlFor={`comment-${idx}`} className="form-label">Your Comment for this section:</label>
                                        <textarea
                                            id={`comment-${idx}`}
                                            className="form-control"
                                            rows="3"
                                            value={reviewContent.find(rc => rc.index === idx)?.comment || ''}
...
function Review() {
        ...
        axios
            ...
            .then((response) => {
                ...
                response.data.forEach(review => {
                    if (review.hasBeenReviewed === false)
                        count = count + 1;
                });
                setNewCount(count);
            })
    ...
    return (
        <div className='container-fluid px-4 py-3 mt-lg-4 mt-2'>
            {reviews.length === 0 ? (
                <>
                    <h2 className="text-center fs-italic fs-4">No articles assigned for review</h2>
                </>
            ) : (
                <>
                    <h2 className="fs-5 fw-semibold">Latest Assigned Articles <span className="badge text-primary">{newCount}</span></h2>
                    <hr />
                        ...
                        {reviews.map((review) => (
                            <div key={review._id}>
                                {review.hasBeenReviewed === false && (
                                    <div className='col-md-3 rounded-3 bg-danger-subtle p-3'>
                                        ...
                                        <div className='d-flex gap-2'>
                                            ...
                                            <h3 className='fs-5'>{review.articleId.author} </h3>
                                        </div>
                                        <div className='d-flex gap-2'>
                                            <h2 className="fs-5 fw-semibold">Island: </h2>
                                            <h3 className='fs-5'>{review.articleId.island} </h3>
                                        </div>
                                        <div className='d-flex gap-2'>
                                            <h2 className="fs-5 fw-semibold">Published On: </h2>
                                            <h3 className='fs-5'>{formatDateOrToday(review.articleId.createdAt)} </h3>
                                        </div>
                                        <hr />
                                        <div className="flex-center">
                                            <button
                    ...
                    <h2 className="fs-5 fw-semibold">Previously Reviewed Articles <span className="badge text-primary">{reviews.length - newCount}</span></h2>
                    <hr />
                    <div className="row flex-jcenter">
            ...
            {showReviewsModal && reviewArticle && (
                <ReviewDisplayModal article={reviewArticle} close={handleCloseReviewsModal} />
            )}
            {reviewMode && <ReviewMode article={reviewArticle} refresh={getAssignments} close={hideReviewMode} />}
        </div>
    )
}
