...
import {
  removeArticleReviews,
  removeReviewById,
} from "../controllers/articleReview.js";
...
router.get("/", async (req, res) => {
  try {
    ...
    if (!reviews) return res.status(403).json({ error: "No Reviews Found!" });
...
router.get("/article/:id", async (req, res) => {
  ...
  try {
    ...
    if (!reviews || reviews.length === 0) {
      // Check for empty array instead of !reviews for better semantics
      return res
        .status(404)
        .json({ message: "No reviews found for this article." });
    }
  ...
  try {
    ...
    if (review.userId !== userId)
      return res.status(400).json({
        error: "Sorry! you do not have the credentials to edit the review",
      });
